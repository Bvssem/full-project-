...
function UserGrades() {
  ...
  useEffect(() => {
    const fetchGrades = async () => {
      const result = await axios.get(`/api/grades/user/${userId}`);
        ...
        {grades.map(grade => (
          <li key={grade.id} className="grade-item">
            <div className="grade-details">
              <span className="assignment-title">Assignment: {grade.assignment.title}</span>
              <span className="grade-score">Grade: {grade.score}</span>
            </div>
          </li>
        ))}
      </ul>

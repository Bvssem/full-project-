...
function StudentQuizAttempts() {
    ...
    useEffect(() => {
        axios.get(`/api/quizAttempts/user/${userId}`)
            .then(response => setQuizAttempts(response.data))
            .catch(error => console.error('Error fetching quiz attempts for current user:', error));
    }, [userId]);
            ...
            <table>
                ...
                <tbody>
                    {quizAttempts.map(quizAttempt => (
                        <tr key={quizAttempt.id}>
                            <td>{quizAttempt.quizTitle}</td>

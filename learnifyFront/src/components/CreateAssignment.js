...
const [selectedCourseId, setSelectedCourseId] = useState('');
  ...
  const handleSubmit = async (event) => {
    ...
    const assignment = { title, questions: questions.map(text => ({ text })) };
    axios.post(`http://localhost:8080/api/assignments/${userId}/${selectedCourseId}`, assignment)
            .then(response => {
...
">
      ...
      <form onSubmit={handleSubmit} className="assignment-form">
        ...
        <button type="button" onClick={handleAddQuestion} className="add-question-button">Add Question</button>

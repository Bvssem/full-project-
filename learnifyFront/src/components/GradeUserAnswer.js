...
import './GradeUserAnswer.css'; // Ensure you have this CSS file for custom styles
...
function GradeUserAnswer() {
  ...
  useEffect(() => {
    const fetchAnswers = async () => {
      const result = await axios.get(`/api/answers/user/${userId}/assignment/${assignmentId}`);
  ...
  const handleGradeSubmit = async (event) => {
    ...
    await axios.post(`/api/grades/submit?userId=${userId}&instructorId=${instructorId}`, {
      score: grade,
...
">
      ...
      <h2>Grade User {userId} for Assignment {assignmentId}</h2>
      <form onSubmit={handleGradeSubmit} className="grade-form">
        ...
        <div className="grade-input-block">
...
export default GradeUserAnswer;

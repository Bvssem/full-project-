...
import './UserAnswersList.css'; // Ensure you have this CSS file for custom styles

function UserAnswersList() {
  const { id } = useParams();
  const [answers, setAnswers] = useState([]);

  useEffect(() => {
    const fetchAnswers = async () => {
      const result = await axios.get(`/api/answers/assignment/${id}`);
  ...
  return (
     ...
     <div>
      ...
      <h2>User Answers for Assignment {id}</h2>
      <ul className="answers-list">
        {answers.map(answer => (
          <li key={answer.id} className="answer-item">
            <Link to={`/grade/${answer.userId}/assignment/${id}`} className="answer-link">
              <span className="username">{answer.username}:</span> {answer.answer}
            </Link>
          </li>
        ))}
      </ul>

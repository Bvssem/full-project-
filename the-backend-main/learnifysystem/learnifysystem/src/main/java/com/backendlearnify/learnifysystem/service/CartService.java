...
public class CartService {
    ...
    public Cart updateCart(Long cartId, Long courseId, int quantity) {
        Cart cart = getCart(cartId);
        Optional<CartItem> optionalCartItem = cart.getItems().stream().filter(item -> item.getCourse().getId().equals(courseId)).findFirst();
        if (optionalCartItem.isPresent()) {
            ...
            throw new RuntimeException("Course not found in the cart");
        }
    }
}
